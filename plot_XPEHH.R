...
for (p2 in 1:length(Populations_List2)) {
	...
	for (p1 in 1:length(Populations_List1)) {
		...
		# wg.xpehh <- readRDS(file= paste0("rehh_outputs/",pop1,"_vs_",pop2,"_xpehh_data.rds")) 
		# If you saved the data you don't need to re-calculate wg.xpehh again.
		
		# write_xlsx(wg.xpehh,"/home/martina/rehh_2022/rehh_outputs_adjusted-Pvalue/",pop1,"_vs_", "_XPEHH.xlsx")
		...
		par(mfrow=c(2,1))
		manhattanplot(wg.xpehh, pval= T, main= paste(genes,collapse=' '), threshold= c(-thres,thres), cex= 0.6, adj= 0.5, font.main=4)
		title(paste0("XPEHH ",pop1), adj= 0.01, font.main=2)
